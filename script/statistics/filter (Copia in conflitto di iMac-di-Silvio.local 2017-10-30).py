...
def occ(base_occ_dir, context_file, use_data=True, use_prov=True):
    print "Start occ"
    final_csv = []
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-nd", "--no-data", dest="no_data", action="store_true", default=False,
                            help="Say that the statistics of the OCC must not consider data.")
    arg_parser.add_argument("-np", "--no-prov", dest="no_prov", action="store_true", default=False,
                            help="Say that the statistics of the OCC must not consider provenance.")
    ...
    if args.sum_column is not None:
        ...
        final_result = occ(args.input, args.context, not args.no_data, not args.no_prov)
